...
class BeamerTranslator (LaTeXTranslator):
    ...
    def depart_document(self, node):
        ...
        if self.author_stack:
            ...
            docinfo_author=tuple([shortauthor,' \\and\n'.join(authors)])
            self.body_pre_docinfo.append(docinfo_w_author%docinfo_author)

        if self.organization is not None:
            docinfo_organization=self.organization
            self.body_pre_docinfo.append(docinfo_w_organization%docinfo_organization)

        if self.date:
            docinfo_date=', '.join(self.date)
            self.body_pre_docinfo.append(docinfo_w_date%docinfo_date)
        ...
        if self.title:
            ...
            docinfo_title = tuple([shorttitle,'%\n  '.join(title)])

            self.body_pre_docinfo.append(PreambleCmds.documenttitle%docinfo_title)
        # b) bibliography
